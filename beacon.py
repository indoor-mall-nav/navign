...
class BeaconUnlockLogic(Scene):
    ...
    def setup_components(self):
        ...
        # Beacon box
        beacon_box = Rectangle(width=3, height=4, color=RED, stroke_width=3, fill_opacity=0.1)
        beacon_label = Tex("ESP32-C3", font_size=20, color=RED).next_to(beacon_box.get_top(), DOWN, buff=0.2)
        
        # Components inside beacon
        human_sensor = Rectangle(width=1.2, height=0.6, color=BLUE, stroke_width=2, fill_opacity=0.2)
        human_label = Tex("Human\\\\Sensor", font_size=12, color=BLUE)
        human_group = VGroup(human_sensor, human_label.move_to(human_sensor.get_center()))
        human_group.move_to(beacon_box.get_top() + DOWN*1)
        
        relay = Rectangle(width=1.2, height=0.6, color=ORANGE, stroke_width=2, fill_opacity=0.2)
        ...
        timer_label = Tex("Timer: 0s", font_size=14, color=GRAY).move_to(beacon_box.get_bottom() + UP*0.5)
        
        beacon_full = VGroup(beacon_box, beacon_label, human_group, relay_group, timer_label)
        beacon_full.shift(LEFT*3)
        
        # Door/lock
        door = Rectangle(width=1.5, height=3, color=GRAY, stroke_width=3, fill_opacity=0.15)
        lock_icon = Tex("Locked", font_size=40).move_to(door.get_center())
        door_group = VGroup(door, lock_icon)
        door_group.shift(RIGHT*3)
        
        # Connection wire
        wire = DashedLine(relay_group.get_right(), door_group.get_left(), color=ORANGE, stroke_width=2)
    ...
    def show_unlock_sequence(self):
        ...
        detection = Tex("Human Detected!", font_size=16, color=YELLOW).next_to(self.human_sensor, RIGHT, buff=0.3)
        pulse = Circle(radius=0.4, color=BLUE, stroke_width=3).move_to(self.human_sensor.get_center())
        ...
        relay_signal = Tex("Relay HIGH", font_size=14, color=ORANGE).next_to(self.relay, LEFT, buff=0.3)
        ...
        unlock_icon = Tex("Unlocked", font_size=40).move_to(self.door_group.get_center())
        ...
        for i, (time, label) in enumerate(zip(timers, labels)):
            new_timer = Tex(f"Timer: {time}", font_size=14, color=YELLOW if i < 5 else RED)
            new_timer.move_to(self.timer_label.get_center())
            
            status = Tex(label, font_size=11, color=GRAY).next_to(new_timer, DOWN, buff=0.1)
        ...
        close_signal = Tex("Relay LOW", font_size=14, color=GRAY).next_to(self.relay, LEFT, buff=0.3)
        ...
        lock_icon_new = Tex("Locked", font_size=40).move_to(self.door_group.get_center())
