...
class PathfindingVisualization(Scene):
    ...
    def area_level_routing(self):
        ...
        conn_bc = Arrow(area_b.get_right(), area_c.get_left(), color=WHITE, buff=0.1, stroke_width=2)
        ...
        # Fade out
        self.play(
            FadeOut(areas), FadeOut(conns), FadeOut(graph_highlight), 
            FadeOut(route_text), FadeOut(server_label),
            run_time=0.4
        )
    ...
    def polygon_quantification_and_routing(self):
        ...
        for i in range(len(x_coords) - 1):
            for j in range(len(y_coords) - 1):
                ...
                is_inside = self.is_inside_polygon(center_x, center_y, polygon_points[:-1])
