...
class NavignOutro(Scene):
    ...
    def show_architecture(self):
        ...
        # Rust badge
        rust_badge = (
            VGroup(Tex("", font_size=28), Tex("100\% Rust", font_size=14, color=ORANGE))
        ...
        mobile = self.create_component(
            "", "Mobile", "Tauri framework\\\\Max Rust integration", BLUE
        ).shift(LEFT * 3.5 + UP * 0.2)

        server = self.create_component(
            "", "Server", "Axum + MongoDB\\\\Service trait pattern", GREEN
        ).shift(UP * 0.2)

        beacon = self.create_component(
            "", "Beacon", "ESP32-C3\\\\esp-hal bare-metal", RED
        ...
        self.play(
            LaggedStart(*[FadeIn(c) for c in components], lag_ratio=0.15), run_time=1.0
        )
    ...
    def show_github(self):
        ...
        github_box = Rectangle(
            width=11, height=1.6, color=WHITE, stroke_width=2, fill_opacity=0.05
        )
        ...
        self.play(Create(github_box), Write(github_icon), run_time=0.5)
        self.play(LaggedStart(*[Write(r) for r in repos], lag_ratio=0.2), run_time=1.2)

        self.github_group = VGroup(github_box, github_icon, repos)

    def show_future(self):
        """Show future roadmap"""
        future_box = Rectangle(
            width=11,
            height=1.3,
            color=GRAY,
            stroke_width=1.5,
        ...
        self.play(Create(future_box), Write(future_title), run_time=0.4)
        self.play(LaggedStart(*[Write(p) for p in plans], lag_ratio=0.3), run_time=1.0)

    def show_cta(self):
        """Final call to action"""
        cta = Tex(
            "Ethan Wu – github.com/7086cmd", font_size=16, color=BLUE, slant=ITALIC
        )
    ...
    def create_component(self, icon, name, details, color):
        ...
        box = Rectangle(
            width=2.3, height=1.6, color=color, stroke_width=2, fill_opacity=0.1
        )
