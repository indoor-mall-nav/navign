...
impl<'a> ProtocolHandler<'a> {
    ...
    pub async fn send_odometry(&mut self, odom: &Odometry) -> Result<(), ProtocolError> {
        ...
        let serialized =
            postcard::to_vec::<Odometry, 256>(odom).map_err(ProtocolError::PostcardError)?;
        self.uart
            .write(serialized.as_slice())
            .await
            .map_err(ProtocolError::UartError)?;
        Ok(())
    }
    ...
    pub async fn receive_command(&mut self) -> Result<MotionCommand, ProtocolError> {
        ...
        let command: MotionCommand =
            postcard::from_bytes(&buffer).map_err(ProtocolError::PostcardError)?;
        Ok(command)
    }
}
