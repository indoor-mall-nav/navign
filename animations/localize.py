...
class LocalizationVisualization(Scene):
    ...
    def ble_scanning(self):
        """BLE beacon scanning"""
        step_label = (
            Tex("1. BLE Scan (5s)", font_size=20, color=BLUE)
            .to_edge(UP)
            .shift(DOWN * 0.5)
        )
        ...
        # Phone
        phone = Rectangle(
            width=0.6, height=1.0, color=BLUE, stroke_width=3, fill_opacity=0.2
        )
        ...
        for pos, rssi, label in beacon_data:
            ...
            rssi_text = Tex(f"{rssi}dBm", font_size=12, color=WHITE).next_to(
                beacon, UP, buff=0.1
            )
            label_text = Tex(label, font_size=10, color=GRAY).next_to(
                beacon, DOWN, buff=0.05
            )

            # Signal waves
            wave = Circle(
                radius=0.3, color=GREEN, stroke_width=1.5, fill_opacity=0
            ).move_to(pos)
    ...
    def area_selection(self):
        ...
        self.play(
            Transform(
                self.step_label,
                Tex("2. Area Selection", font_size=20, color=GREEN)
                .to_edge(UP)
                .shift(DOWN * 0.5),
            ),
            run_time=0.3,
        ...
        area_a = Rectangle(
            width=3, height=3.5, color=BLUE, stroke_width=2, fill_opacity=0.05
        ).shift(LEFT * 2)
        area_b = Rectangle(
            width=3, height=3.5, color=ORANGE, stroke_width=2, fill_opacity=0.05
        ).shift(RIGHT * 2)

        area_a_label = Tex("Area A (3 beacons)", font_size=14, color=BLUE).next_to(
            area_a, UP, buff=0.1
        )
        area_b_label = Tex("Area B (2 beacons)", font_size=14, color=ORANGE).next_to(
            area_b, UP, buff=0.1
        )
        ...
        selection_box = SurroundingRectangle(
            area_a, color=YELLOW, stroke_width=4, buff=0.1
        )
        selection_text = Tex("Area A selected", font_size=16, color=YELLOW).to_edge(
            DOWN
        )

        self.play(Create(selection_box), Write(selection_text), run_time=0.8)
        ...
        self.play(
            ...
            FadeOut(
                VGroup(*[b["wave"] for b in self.beacons[1:]])
            ),  # Keep only Area A beacons
            FadeOut(self.beacons[1]["beacon"]),
            FadeOut(self.beacons[1]["rssi_text"]),
            FadeOut(self.beacons[1]["label"]),
            FadeOut(self.beacons[3]["beacon"]),
            FadeOut(self.beacons[3]["rssi_text"]),
            FadeOut(self.beacons[3]["label"]),
            run_time=0.4,
        )
    ...
    def position_calculation(self):
        ...
        self.play(
            Transform(
                self.step_label,
                Tex("3. Position Calculation", font_size=20, color=PURPLE)
                .to_edge(UP)
                .shift(DOWN * 0.5),
            ),
            run_time=0.3,
        ...
        calc_label = (
            Tex("Weighted Centroid Method", font_size=16, color=BLUE)
            .to_edge(DOWN)
            .shift(UP * 2)
        )
        ...
        formula = (
            ...
            .scale(0.6)
            .next_to(calc_label, DOWN, buff=0.2)
        )
        ...
        for b in self.active_beacons:
            ...
            weight_text = (
                Tex(f"w={weight:.2f}", font_size=10, color=BLUE)
                .move_to((self.phone_group.get_center() + b["beacon"].get_center()) / 2)
                .shift(UP * 0.2)
            )
        ...
        result_circle = Circle(radius=0.3, color=YELLOW, stroke_width=3).move_to(
            final_pos
        )
        coords_text = Tex(
            f"({final_x:.1f}, {final_y:.1f})", font_size=14, color=YELLOW
        ).next_to(result_marker, DOWN, buff=0.3)
