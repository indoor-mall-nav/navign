...
class DatabaseSchema(Scene):
    def construct(self):
        ...
        self.play(
            LaggedStart(*[GrowArrow(a) for a in refs], lag_ratio=0.1), run_time=1.5
        )  # 1.5s: draw arrows
        ...
        self.play(
            Create(wkt_highlights), FadeIn(wkt_note), run_time=1.5
        )  # 1.5s: WKT highlight
    ...
    def create_box(self, name, color, detail):
        ...
        box = Rectangle(
            width=2, height=0.9, color=color, stroke_width=2, fill_opacity=0.1
        )
