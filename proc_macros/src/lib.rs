...
pub fn derive_sql_repository(input: TokenStream) -> TokenStream {
    ...
    // Extract attributes
    let table_name = extract_attr(&input.attrs, "table").unwrap_or_else(|| {
        panic!("#[repo(table = \"...\")] attribute is required")
    });
    
    let search_fields = extract_attr(&input.attrs, "search_fields").unwrap_or_else(|| {
        panic!("#[repo(search_fields = \"...\")] attribute is required")
    });
    
    let select_fields = extract_attr(&input.attrs, "select_fields").unwrap_or_else(|| {
        panic!("#[repo(select_fields = \"...\")] attribute is required")
    });
    ...
    let default_sort = extract_attr(&input.attrs, "default_sort")
    ...
    let postgres_impl = if has_entity {
        generate_postgres_with_entity(name, &table_name, &select_fields, &search_field_list, &default_sort)
    } else {
        generate_postgres_without_entity(name, &table_name, &select_fields, &search_field_list, &default_sort)
    };
    
    // Generate SQLite search implementation
    let sqlite_impl = if has_entity {
        generate_sqlite_with_entity(name, &table_name, &select_fields, &search_field_list, &default_sort)
    } else {
        generate_sqlite_without_entity(name, &table_name, &select_fields, &search_field_list, &default_sort)
    };
...
fn generate_postgres_with_entity(
    ...
    let like_conditions = search_fields.iter()
