...
pub trait Navigate<'a> {
    fn navigate(&self, departure: Location<'a>, arrival: Location<'a>, limits: ConnectivityLimits, allocator: &'a Bump) -> NavigationResult;
}

impl<'a> Navigate<'a> for Entity<'a> {
    fn navigate(&self, departure: Location<'a>, arrival: Location<'a>, limits: ConnectivityLimits, allocator: &'a Bump) -> NavigationResult {
        if departure.2 == arrival.2 {
            let area = match self.get_areas().iter().find(|area| area.database_id == departure.2) {
                Some(area) => area,
                None => return Err(NavigationError::InvalidDeparture),
            };
            let route = match area.polygon.as_bounded_block_array().find_displacement(
            ...
            info!("Finding path from area {} to area {}", departure.2, arrival.2);
            let mut instructions = bumpalo::collections::Vec::new_in(allocator);
            let connectivity = match self.find_path(departure.2, (departure.0, departure.1), arrival.2, limits, &allocator) {
                Some(conn) => conn,
                None => return Err(NavigationError::NoRoute),
            };
            ...
            for window in conns.windows(2) {
                ...
                println!("From area {area_src_id} via connection {conn_dest_id} to area {area_dest_id}");
                let area_src = match self.get_areas().iter().find(|area| area.database_id == *area_src_id) {
                    Some(area) => area,
                    None => return Err(NavigationError::InvalidDeparture),
                };
                let conn = match area_src.connections.iter().find(|conn| conn.database_id == *conn_dest_id) {
                    Some(conn) => conn,
                    None => return Err(NavigationError::Other("Connection not found in source area".to_string())),
                };
                let (_, x, y) = match conn.connected_areas.iter().find(|(a, _, _)| a.database_id == *area_src_id) {
                    Some((a, x, y)) => (a, *x, *y),
                    None => return Err(NavigationError::Other("Connected area not found in connection".to_string())),
                };
                println!("  Walking inside area {area_src_id} from ({target_x}, {target_y}) to ({x}, {y})");
                let inner_route = area_src.polygon.as_bounded_block_array().find_displacement(
                    (target_x, target_y),
                    (x, y),
                );
                println!("  Inner route: {inner_route:?}");
                (_, target_x, target_y) = match conn.connected_areas.iter().find(|(a, _, _)| a.database_id == *area_dest_id) {
                    Some((a, x, y)) => (a, *x, *y),
                    None => return Err(NavigationError::Other("Connected area not found in connection".to_string())),
                };
                if let Some(inner_route) = inner_route {
                    println!("  Inner route instructions: {:?}", inner_route.as_instructions());
                    instructions.extend(inner_route.as_instructions());
                } else {
                    return Err(NavigationError::NoRoute);
                }
                instructions.push(InstructionType::Transport(conn.database_id.to_string(), *conn.r#type));
            }
            let area_dest = match self.get_areas().iter().find(|area| area.database_id == arrival.2) {
                Some(area) => area,
                None => return Err(NavigationError::InvalidArrival),
            };
            println!("Final walk inside area {}, from ({target_x}, {target_y}) to ({}, {})", arrival.2, arrival.0, arrival.1);
            let final_route = area_dest.polygon.as_bounded_block_array().find_displacement(
                (target_x, target_y),
                (arrival.0, arrival.1),
            );
