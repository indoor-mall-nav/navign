...
describe('Authentication APIs', () => {
  ...
  it('should logout user', async () => {
    ...
    expect(result.status).toBe('success');
    expect(invoke).toHaveBeenCalledWith('logout_handler', { token: 'test_token' });
  });
});
...
describe('Route/Navigation APIs', () => {
  ...
  it('should calculate total distance correctly', async () => {
    const mockRouteResponse: ApiResponse<RouteResponse> = {
      ...
      data: {
        instructions: [
          { type: 'move', area: 'area1', from: [0, 0], to: [10, 0], distance: 10 },
          { type: 'move', area: 'area1', from: [10, 0], to: [10, 10], distance: 10 }
        ],
        total_distance: 20,
        areas: ['area1']
      }
