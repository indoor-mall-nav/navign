    ...
    pub fn serialize_message(&mut self, message: &BleMessage) -> Result<&[u8], BleError> {
        ...
        match message {
            BleMessage::NonceRequest => {
                self.send_buffer.push(0x01).map_err(|_| BleError::BufferFull)?;
            }

            BleMessage::NonceResponse(nonce) => {
                self.send_buffer.push(0x02).map_err(|_| BleError::BufferFull)?;
                self.send_buffer.extend_from_slice(nonce.as_bytes()).map_err(|_| BleError::BufferFull)?;
            }

            BleMessage::ProofSubmission(proof) => {
                self.send_buffer.push(0x03).map_err(|_| BleError::BufferFull)?;
                self.send_buffer.extend_from_slice(&proof.challenge_hash).map_err(|_| BleError::BufferFull)?;
                self.send_buffer.extend_from_slice(&proof.device_signature).map_err(|_| BleError::BufferFull)?;
                self.send_buffer.extend_from_slice(&proof.timestamp.to_be_bytes()).map_err(|_| BleError::BufferFull)?;
                self.send_buffer.extend_from_slice(&proof.counter.to_be_bytes()).map_err(|_| BleError::BufferFull)?;
            }

            BleMessage::UnlockResult(success) => {
                self.send_buffer.push(0x04).map_err(|_| BleError::BufferFull)?;
                self.send_buffer.push(if *success { 0x01 } else { 0x00 }).map_err(|_| BleError::BufferFull)?;
            }
        }
    ...
    pub fn deserialize_message(&mut self, data: &[u8]) -> Result<BleMessage, BleError> {
        ...
        match self.receive_buffer[0] {
            ...
            0x03 => {
                ...
                let timestamp = u64::from_be_bytes(self.receive_buffer[97..105].try_into().unwrap());
