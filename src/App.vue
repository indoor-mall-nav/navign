...
async function useBLEC() {
  try {
    ...
    let ID = result.find(x => x.name.includes('BEACON'))?.address ?? "f3c0db4c-bf08-6332-c5dc-995b759b556c";
    await connect(ID, null, true);
    console.log("Connected to BLEC device");

    const dataToSend = new Uint8Array([0xFF, 0x01, 0x02, 0x03, 0x04]);
    await send(
...
<template>
  <main class="container">
    <Button @click="useBLEC">Use BLEC Plugin</Button>
    <RouterView />
  </main>
</template>
