...
function generateSocialMediaUrl(
  media: NonNullable<Merchant["social_media"]>[0],
): {
  type: "url" | "qrcode";
  url?: string;
} {
  ...
  const platform =
    typeof media.platform === "string" ? media.platform : "other";
  switch (platform) {
    case "twitter":
...
function processTodayOpeningHours(
  ...
  if (!opening_hours) return "Closed";
  ...
  if (dayOfWeek < opening_hours.length) {
    ...
    const [start, end] = todayHours[0]; // Take first time slot
    return processOpeningHours([start, end]);
  }
  return "Closed";
}
</script>
  ...
  <Card class="my-2">
    ...
    <CardContent>
      <p class="flex">
        ...
        <span v-if="merchant.opening_hours" class="font-medium">
          <span
            v-if="
              merchant.opening_hours &&
              merchant.opening_hours[new Date().getDay()] &&
              merchant.opening_hours[new Date().getDay()].length > 0 &&
              new Date().getHours() >=
                (merchant.opening_hours[new Date().getDay()][0][0] ??
                  86400000) /
                  3600000 &&
              new Date().getHours() <
                (merchant.opening_hours[new Date().getDay()][0][1] ?? 0) /
                  3600000
            "
      ...
      <div class="flex mx-4">
        ...
        <Button
          v-for="(social, index) in (merchant.social_media || []).map((x) => ({
            ...
            platformName:
              typeof x.platform === 'string' ? x.platform : x.platform.other,
          }))"
          :key="index"
          variant="link"
          size="icon"
        ...
        >
          <Icon
            :icon="`tabler:brand-${social.platformName}`"
            class="w-5 h-5 mr-2"
          />
        </Button>
      </div>
