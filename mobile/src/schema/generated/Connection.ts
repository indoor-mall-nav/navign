// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { ConnectionType } from "./ConnectionType";

/**
 * Connection schema - represents connections between areas (gates, elevators, etc.)
 */
export type Connection = { id: number, entity_id: string, name: string, description: string | null, type: ConnectionType, 
/**
 * List of Area IDs that this connection links
 */
connected_areas: Array<[string, number, number, boolean]>, 
/**
 * List of `(start_time, end_time)` in milliseconds on a 24-hour clock
 */
available_period: Array<[number, number]>, tags: Array<string>, gnd: [number, number] | null, created_at: bigint | null, updated_at: bigint | null, };
