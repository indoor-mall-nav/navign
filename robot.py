...
class RobotArchitecture(Scene):
    def construct(self):
        ...
        # Title with LaTeX
        title = Tex(
            r"\textbf{Dual-Purpose Robot Dog Architecture}",
            font_size=40
        ).to_edge(UP)
        ...
        stm32_text = Tex(
            r"\texttt{STM32F4/H7} \\ \texttt{Embassy}",
            font_size=22,
            color=WHITE
        ...
        serial_label = Tex(
            r"\texttt{UART} \\ $50~\mathrm{Hz}$",
            font_size=14,
            color=WHITE
        ).next_to(serial_connection, LEFT, buff=0.1).shift(DOWN * 0.3)
        ...
        # ===== DATA FLOW ANIMATION =====
        flow_title = Tex(
            r"\textbf{Guide Mode:} \textit{Data Flow}",
            font_size=32,
            color=YELLOW
        ).to_edge(UP)
        ...
        for idx, label, color in flow_sequence:
            ...
            # Flash the subsystem
            self.play(
                box.animate.set_fill(color, opacity=0.6),
                run_time=0.2
            )
        ...
        self.play(
            stm32_box.animate.set_fill(GREEN, opacity=0.6),
            run_time=0.3
        )
        
        stm32_desc = Tex(
            r"\textit{Execute motor control}",
            font_size=20,
            color=GREEN
        ...
        self.play(
            mcpwm_circle.animate.set_fill(GREEN, opacity=0.6),
            run_time=0.3
        )
        
        motor_arrows = VGroup(
            Arrow(mcpwm_circle.get_bottom(), mcpwm_circle.get_bottom() + DOWN * 0.5, color=GREEN),
            Arrow(mcpwm_circle.get_right(), mcpwm_circle.get_right() + RIGHT * 0.5, color=GREEN),
        )
        motor_label = Tex(
            r"\textit{Wheels turn}",
            font_size=16,
            color=GREEN
        ).next_to(motor_arrows, DOWN)
        ...
        guide_mode = Tex(
            ...
            font_size=24,
            color=BLUE
        ).move_to(LEFT * 4 + UP * 3.5)
        delivery_mode = Tex(
            r"\textsc{Delivery Mode}",
            font_size=24,
            color=RED
        ).move_to(RIGHT * 4 + UP * 3.5)
        
        self.play(Write(guide_mode), Write(delivery_mode))
        self.wait(0.3)
        
        # Show priority differences
        guide_priority = VGroup(
        ...
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.15).next_to(guide_mode, DOWN, buff=0.3)
        
        delivery_priority = VGroup(
        ...
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.15).next_to(delivery_mode, DOWN, buff=0.3)
...
class DataFlowDetailed(Scene):
    def construct(self):
        # Title
        title = Tex(
            r"\textbf{Guide Scenario:} \texttt{``Take me to Starbucks''}",
            font_size=36
        ).to_edge(UP)
        self.play(Write(title))
        self.wait(0.5)
        ...
        steps = [
            (r"$\mathbf{1.}$ \textsc{Audio Wake Word}", ORANGE, r"\texttt{User: `Take me to Starbucks'}"),
            (r"$\mathbf{2.}$ \textsc{Tasks Dispatcher}", RED, r"Dispatch guide mission"),
            (r"$\mathbf{3.}$ \textsc{Navign Server}", TEAL, r"Route: $\text{Current} \to \text{Starbucks}$"),
            (r"$\mathbf{4.}$ \textsc{Bluetooth Scan}", PURPLE, r"BLE: Zone A3, near escalator"),
            (r"$\mathbf{5.}$ \textsc{Vision AprilTag}", YELLOW, r"Precise pose at intersection"),
            (r"$\mathbf{6.}$ \textsc{Vision YOLO}", YELLOW, r"Detect: Person at $d = 2~\mathrm{m}$"),
            (r"$\mathbf{7.}$ \textsc{Tasks LLM}", RED, r"Reason: Slow down, be polite"),
            (r"$\mathbf{8.}$ \textsc{Audio Output}", ORANGE, r"\texttt{`Excuse me, passing left'}"),
            (r"$\mathbf{9.}$ \textsc{Serial UART}", PINK, r"Cmd: $v = 0.3~\mathrm{m/s}$, $\omega = -0.1~\mathrm{rad/s}$"),
        ...
        for i, (step_name, color, description) in enumerate(steps):
            ...
            # Step box
            box = RoundedRectangle(
                width=3.5, height=0.5, corner_radius=0.05,
                color=color, fill_opacity=0.3
            ).move_to(LEFT * 3 + UP * y_pos)
            
            step_text = Tex(step_name, font_size=16, color=color).move_to(box.get_center())
            
            # Description
            desc_text = Tex(description, font_size=14, color=WHITE).next_to(box, RIGHT, buff=0.3)
...
class LocalizationHierarchy(Scene):
    def construct(self):
        # Title
        title = Tex(
            r"\textbf{Multi-Tier Localization Strategy}",
            font_size=36
        ).to_edge(UP)
        self.play(Write(title))
        self.wait(0.5)
        ...
        for name, purpose, accuracy, rate, color, position in tiers:
            # Box
            box = RoundedRectangle(
                width=6, height=1.2, corner_radius=0.1,
                color=color, fill_opacity=0.2
            ).move_to(position)
        ...
        fusion_title = Tex(
            r"\textbf{Extended Kalman Filter Fusion:}",
            font_size=24
        ...
        state_vector = MathTex(
            ...
            font_size=18
        ).to_edge(DOWN).shift(LEFT * 3)
        
        state_label = Tex(
            r"\textit{State vector}",
            font_size=14
        ).next_to(state_vector, DOWN, buff=0.1)
...
class ControlLoop(Scene):
    def construct(self):
        # Title
        title = Tex(
            r"\textbf{Low-Level Control:} \texttt{STM32 + Embassy}",
            font_size=36
        ).to_edge(UP)
        self.play(Write(title))
        self.wait(0.5)
        ...
        # Control blocks
        setpoint = RoundedRectangle(width=2, height=0.6, color=GREEN, fill_opacity=0.3).shift(LEFT * 4 + UP * 1)
        setpoint_text = Tex(r"$v_{\text{target}}$", font_size=20).move_to(setpoint.get_center())
        
        error_circle = Circle(radius=0.4, color=RED, fill_opacity=0.3).shift(LEFT * 2 + UP * 1)
        error_text = MathTex(r"e = v_{\text{target}} - v_{\text{actual}}", font_size=14).move_to(error_circle.get_center())
        
        pid_box = RoundedRectangle(width=2.5, height=0.8, color=BLUE, fill_opacity=0.3).shift(UP * 1)
        pid_text = MathTex(
            r"K_p e + K_i \int e \, dt + K_d \frac{de}{dt}",
            font_size=14
        ).move_to(pid_box.get_center())
        
        motor_box = RoundedRectangle(width=1.8, height=0.6, color=ORANGE, fill_opacity=0.3).shift(RIGHT * 3 + UP * 1)
        motor_text = Tex(r"\texttt{MCPWM}", font_size=18).move_to(motor_box.get_center())
        
        encoder_box = RoundedRectangle(width=1.8, height=0.6, color=PURPLE, fill_opacity=0.3).shift(RIGHT * 3 + DOWN * 0.5)
        encoder_text = Tex(r"$v_{\text{actual}}$", font_size=18).move_to(encoder_box.get_center())
        
        # Arrows
        arrow1 = Arrow(setpoint.get_right(), error_circle.get_left(), color=WHITE, stroke_width=3)
        arrow2 = Arrow(error_circle.get_right(), pid_box.get_left(), color=WHITE, stroke_width=3)
        arrow3 = Arrow(pid_box.get_right(), motor_box.get_left(), color=WHITE, stroke_width=3)
        arrow4 = Arrow(motor_box.get_bottom(), encoder_box.get_top(), color=WHITE, stroke_width=3)
        arrow5 = Arrow(encoder_box.get_left(), encoder_box.get_left() + LEFT * 4, color=WHITE, stroke_width=3)
        ...
        self.play(
            FadeIn(setpoint), Write(setpoint_text),
            run_time=0.4
        )
        self.play(Create(arrow1), run_time=0.3)
        self.play(
            FadeIn(error_circle), Write(error_text),
            run_time=0.4
        )
        self.play(Create(arrow2), run_time=0.3)
        self.play(
            FadeIn(pid_box), Write(pid_text),
            run_time=0.4
        )
        self.play(Create(arrow3), run_time=0.3)
        self.play(
            FadeIn(motor_box), Write(motor_text),
            run_time=0.4
        )
        self.play(Create(arrow4), run_time=0.3)
        self.play(
            FadeIn(encoder_box), Write(encoder_text),
            run_time=0.4
        )
